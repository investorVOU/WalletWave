...
try {
    ...
    if (!$isConnected || !$walletAddress) {
        ...
        if ($userId) {
            ...
            $stmt = $pdo->prepare("SELECT address, is_connected FROM user_wallets WHERE user_id = ? AND is_connected = true LIMIT 1");
            $stmt->execute([$userId]);
            ...
            if ($sessionId) {
                $stmt = $pdo->prepare("SELECT address, is_connected FROM guest_wallets WHERE session_id = ? AND is_connected = true LIMIT 1");
